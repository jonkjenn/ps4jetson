...
#define UVC_STREAM_FID	(1 << 0)
    ...
    int USBMgr::listDevices( std::vector<PS4EYECam::PS4EYERef>& list )
    {
        ...
        while ((dev = devs[i++]) != NULL)
        {
            ...
            {
                cout<<"Found PlayStation Camera" << endl;
            ...
            {
                cout<<"Found ov580 boot mode Camera" << endl;
...
*/
  ...
  /*  const uint8_t* PS4EYECam::getLastVideoFramePointer()
    ...
    int PS4EYECam::uvc_get_brightness(uint16_t* brightness, uint8_t  req_code) {
        uint8_t data[2];
        int ret;
    ...
    int PS4EYECam::uvc_set_brightness(uint16_t brightness) {
        uint8_t data[2];
        int ret;
    ...
    bool PS4EYECam::open_usb()
    {
        ...
        uint16_t brightness;
        int sal;
        sal=uvc_get_brightness(&brightness, UVC_GET_CUR);
        cout << sal << "Current brightness " << brightness<< endl;

        int ret = uvc_set_brightness(2);
        sal=uvc_get_brightness(&brightness, UVC_GET_CUR);
        cout << sal << "Current brightness " << brightness<< endl;
        cout << "Ret: " << ret;
        sal=uvc_get_brightness(&brightness, UVC_GET_MIN);
        cout << sal << "Min brightness " << brightness<< endl;


        sal=uvc_get_brightness(&brightness, UVC_GET_MAX);
    ...
    void  LIBUSB_CALL PS4EYECam::ct_done(struct libusb_transfer * transfer) {
        ...
        debug("Controltransfer completed\n");
    ...
    void PS4EYECam::submit_controlTransfer(uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, uint16_t wLength, uint8_t *buffer )
    {
        ...
        int error = libusb_submit_transfer(control_transfer);
        debug("Libusb_submit_transfer %d\n", error);
    }
